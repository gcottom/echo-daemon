...
function shouldSkipForward(url: string): boolean {
    try {
        ...
        // these itags contain audio-only streams, they should always be forwarded
        return itag !== "140" && itag !== "141" && itag !== "249" && itag !== "250" && itag !== "251" && itag !== "599" && itag !== "600";
    } catch {
        return false;
    }
}
...
// and send it to the echo-daemon server to initiate the download.
chrome.webRequest.onBeforeRequest.addListener(
    (details: chrome.webRequest.WebRequestBodyDetails) => {
        ...
        if (details.requestBody && details.requestBody.raw) {
            ...
            try {
                ...
                if (bodyJson.videoId) {
                    fetch(`http://localhost:${goPort}/download/${bodyJson.videoId}`, {
                        method: "GET",
                        headers: { "Content-Type": "application/json" },
